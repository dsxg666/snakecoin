...
// 对于普通的类型，比如字符串，bool型等数据，就是直接往str字段里面填充就行了。
// 但是对于结构体类型的处理，就需要特殊的处理方式了，如 makeStructWriter 方法。
type encBuffer struct {
	str     []byte     // string数据，包含除 list headers 之外的所有内容
	lheads  []listhead // 所有的 list headers
	lhsize  int        // 所有编码的 list headers 的大小之和
	sizebuf [9]byte    // 用于uint编码的辅助buffer
}
...
// size 返回编码数据的长度，即str的长度加上lhsize
...
// Write 实现 io.Writer 并将b直接追加到输出。
func (buf *encBuffer) Write(b []byte) (int, error) {
	buf.str = append(buf.str, b...)
	return len(b), nil
}

// writeXxx 把每种不同的数据填充到encbuf里面去。

// writeBool 将b编码成整数0 (false)或1 (true)。
...
// list 将一个新的 list header 添加到 header 堆栈中。并返回 header 的索引。
// 在对列表内容进行编码后，使用该索引调用 listEnd。
...
func (buf *encBuffer) listEnd(index int) {
	lh := &buf.lheads[index]
	lh.size = buf.size() - lh.offset - lh.size // lh.size 记录了list开始的时候的 list header 应该占用的长度。
	if lh.size < 56 {
...
// encode 首先获取了对象的反射类型，根据反射类型获取它的编码器，然后调用编码器的writer方法。
...
// ToBytes 对listhead进行处理，组装成完整的RLP数据。并返回已编码的字节。
