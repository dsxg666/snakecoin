...
// Decode 从r中解析RLP编码的数据，并将结果存储在val指向的值上。
...
// Bytes 读取RLP字符串并将其内容作为字节切片返回。
// 因为不同长度的字符串有不同方式的编码，我们可以通过前缀的不同来获取字符串的类型，这里我们通过s.Kind()方法获取当前需要解析的类型和长度。
// 如果是Byte类型，那么直接返回Byte的值，如果是String类型那么读取指定长度的值然后返回。这就是kind()方法的用途。
